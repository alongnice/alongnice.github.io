...
// node_modules/.pnpm/overlayscrollbars@2.9.2/node_modules/overlayscrollbars/overlayscrollbars.mjs
...
var isDefaultDirectionScrollCoordinates = ({ D: t2, T: n2 }) => {
...
var getScrollCoordinatesPercent = ({ D: t2, T: n2 }, o2) => {
...
var k = `data-overlayscrollbars`;
var M = "os-environment";
var R = `${M}-scrollbar-hidden`;
var V = `${k}-initialize`;
var L = "noClipping";
var U = `${k}-body`;
var P = k;
...
var createDOMObserver = (t2, n2, o2, s2) => {
  ...
  const observerCallback = (e3, c3) => {
    if (!isEmptyArray(c3)) {
      ...
      let p3 = false;
      each(c3, (o3) => {
        const { attributeName: e4, target: c4, type: a3, oldValue: u3, addedNodes: v3, removedNodes: h3 } = o3;
...
var createObserversSetup = (t2, n2, o2, s2) => {
  ...
  const { vt: v2, gt: p2, ot: g2, ht: h2, bt: b2, nt: w2, wt: y2, yt: S2, St: m2, Ot: O2 } = t2;
  ...
  const [T2, k2] = createDOMObserver(p2, false, onHostMutation, {
    X: d2,
    j: d2
  });
...
var createScrollbarsSetupElements = (t2, n2, o2, s2) => {
  ...
  const { slot: a2 } = i2;
  const { vt: u2, gt: _2, ot: d2, Tt: f2, ht: p2, wt: g2, nt: h2 } = n2;
...
var createScrollbarsSetupEvents = (t2, n2, o2, s2) => (r2, l2, i2) => {
  const { gt: u2, ot: _2, nt: d2, ht: f2, Kt: v2, Ot: p2 } = n2;
...
var createScrollbarsSetup = (t2, n2, o2, s2, e2, c2) => {
  ...
  const [p2, g2] = selfClearTimeout(100);
  ...
  const T2 = [y2, g2, b2, v2, () => _2(), addEventListener(O2, "pointerover", onHostMouseEnter, {
    A: true
...
var createStructureSetupElements = (t2) => {
  ...
  const g2 = isBodyElement(p2);
  const h2 = p2.ownerDocument;
  ...
  const B2 = !elementIsGenerated(T2) && elementHasOverflow(T2) ? T2 : p2;
  const F2 = E2 ? b2 : T2;
  const j2 = E2 ? h2 : T2;
  const X2 = {
    vt: p2,
  ...
  const { vt: J2, gt: Q2, ln: Z2, ot: tt2, bt: nt2 } = X2;
  const ot2 = [() => {
    removeAttrs(Q2, [P, V]);
    removeAttrs(J2, V);
    if (g2) {
...
var createPaddingUpdateSegment = ({ gt: t2, ln: n2, ot: o2, nt: s2 }, e2) => {
...
var createOverflowUpdateSegment = (t2, s2) => {
  ...
  const { gt: c2, ln: r2, ot: l2, nt: a2, Qt: u2, ht: _2, wt: d2, St: f2, un: v2 } = t2;
  const { R: p2 } = e2;
  const g2 = d2 && a2;
  const h2 = bind(o, 0);
  const b2 = {
  ...
  const [T2] = createCache(S2);
  const [k2, M2] = createCache(y2);
  ...
  return ({ It: n2, Zt: o2, _n: i2, Dt: a3 }, { fn: u3 }) => {
    ...
    const [gt2, ht2] = V2(a3);
    const bt2 = b3 || w3 || ht2 || ut2 || a3;
    const [wt2, yt2] = bt2 ? U2(getMeasuredScrollCoordinates(gt2), a3) : N2();
    if (dt2) {
...
var createSetups = (t2, n2, o2, s2, e2) => {
  ...
  const [p2, g2, , h2] = createScrollbarsSetup(t2, n2, v2, a2, u2, e2);
  const updateHintsAreTruthy = (t3) => keys(t3).some((n3) => !!t3[n3]);
  const update = (t3, e3) => {
    if (o2()) {
      return false;
    }
    const { pn: r3, Dt: l3, At: a3, gn: u3 } = t3;
    const _3 = r3 || {};
    const d3 = !!l3 || !c2;
    const p3 = {
...
var OverlayScrollbars = (t2, n2, o2) => {
  ...
  if (n2 && !r2) {
    ...
    const [g2, h2, b2, w2, y2] = createSetups(t2, a2, () => r3, ({ pn: t3, Dt: n3 }, { Zt: o3, tn: s3 }) => {
